// Depending on your application it's pretty unlikely that
// you'll have to type out the entirety of this struct. For
// example, in most cases you won't need division, and you
// can leave out most of the operators too. NB: Thse are
// fairly terrible implementations. Multiplication is about
// twice as slow as Python, and division is about 20 times
// slower. Use only as a last resort when for some reason
// you can't use Java's native bignums.
struct bignum {
  ...
  void mult(const bignum &x) {
    ...
    for (int i = 0; i < digits.size(); i++) {
      for (int j = 0; j < x.digits.size(); j++) {
        unsigned long long y = digits[i];
        y *= x.digits[j];
        unsigned long long z = y / RADIX;
        res[i + j + 1] += z;
        res[i + j] += y - RADIX * z; // mod is slow
        if (res[i + j] >= RADIX) {
          res[i + j] -= RADIX;
          res[i + j + 1]++;
        }
  ...
  bignum div(const bignum &x) {
    ...
    while (pwr >= 0) {
      if (dividend.digits.size() > divisor.digits.size()) {
        ...
        q *= RADIX;
        q += dividend.digits[dividend.digits.size() - 2];
        ...
        assert(dividend.digits.size() <= divisor.digits.size());
        continue;
      }
      while (dividend.digits.size() == divisor.digits.size()) {
        uint q = dividend.digits.back() /
                 (1 + divisor.digits.back());
        if (q == 0)
          break;
        digits[pwr] += q;
        dividend -= divisor * q;
  ...
  bignum &operator%=(const bignum &y) { *this = div(y); }
  ...
  bool operator>(const bignum &y) const { return y < *this; }
